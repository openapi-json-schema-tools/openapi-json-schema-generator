some_api.paths.config_tls_csr_pem.operation
# Operation Method Name

| Method Name | Api Class | Notes |
| ----------- | --------- | ----- |
| config_tls_csr_pem_post | [DefaultApi](../../apis/tags/default_api.md) | This api is only for tag=default |
| post | ApiForPost | This api is only for this endpoint |
| post | ConfigTlsCsrPem | This api is only for path=/config/tls/csr.pem |
...
with some_api.ApiClient(used_configuration) as api_client:
    ...
    try:
        api_response = api_instance.config_tls_csr_pem_post(
            body=body,
        )
        pprint(api_response)
    except some_api.ApiException as e:
        print("Exception when calling DefaultApi->config_tls_csr_pem_post: %s\n" % e)
```
