some_api.paths.users_user_id_passphrase.operation
# Operation Method Name

| Method Name | Api Class | Notes |
| ----------- | --------- | ----- |
| users_user_id_passphrase_post | [DefaultApi](../../apis/tags/default_api.md) | This api is only for tag=default |
| post | ApiForPost | This api is only for this endpoint |
| post | UsersUserIDPassphrase | This api is only for path=/users/{UserID}/passphrase |
...
with some_api.ApiClient(used_configuration) as api_client:
    ...
    body = user_passphrase_post_data.UserPassphrasePostData.validate({
        "passphrase": passphrase.Passphrase.validate("passphrase_example"),
    })
    try:
        api_response = api_instance.users_user_id_passphrase_post(
            body=body,
        )
        pprint(api_response)
    except some_api.ApiException as e:
        print("Exception when calling DefaultApi->users_user_id_passphrase_post: %s\n" % e)
```
