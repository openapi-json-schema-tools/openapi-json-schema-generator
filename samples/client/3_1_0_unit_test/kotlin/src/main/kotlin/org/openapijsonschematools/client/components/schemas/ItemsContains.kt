...
class ItemsContains {
    ...
    class ItemsContainsList(m: FrozenList<Any?>) : FrozenList<Any?>(m) {
        companion object {
            @Throws(ValidationException::class)
            fun of(arg: List<Any?>, configuration: SchemaConfiguration?): ItemsContainsList {
                return ItemsContains1.getInstance().validate(arg, configuration)
            }
        }
    }
    
    class ItemsContainsListBuilder {
        ...
        fun add(item: Nothing?): ItemsContainsListBuilder {
            list.add(null)
            return this
        }
        
        fun add(item: Boolean): ItemsContainsListBuilder {
            list.add(item)
            return this
        }
        
        fun add(item: String): ItemsContainsListBuilder {
            list.add(item)
            return this
        }
        
        fun add(item: Int): ItemsContainsListBuilder {
            list.add(item)
            return this
        }
        
        fun add(item: Float): ItemsContainsListBuilder {
            list.add(item)
            return this
        }
        
        fun add(item: Long): ItemsContainsListBuilder {
            list.add(item)
            return this
        }
        
        fun add(item: Double): ItemsContainsListBuilder {
            list.add(item)
            return this
        }
        
        fun add(item: List<*>): ItemsContainsListBuilder {
            list.add(item)
            return this
        }
        
        fun add(item: Map<String, Any?>): ItemsContainsListBuilder {
            list.add(item)
            return this
        }
    ...
    sealed interface ItemsContains1Boxed {
        fun getData(): Any?
    }
    
    data class ItemsContains1BoxedList(val data: ItemsContainsList) : ItemsContains1Boxed {
        override fun getData(): Any? {
            return data
        }
    }
    ...
    class ItemsContains1 private constructor(): JsonSchema<ItemsContains1Boxed>(
        type = setOf(List::class.java),
        items = Items::class.java,
        contains = Contains::class.java,
    ), ListSchemaValidator<ItemsContainsList, ItemsContains1BoxedList> {
        /*
        ...
        */
        ...
        companion object {
            @Volatile
            private var instance: ItemsContains1? = null
    
            fun getInstance() =
                instance ?: synchronized(this) {
                    instance ?: ItemsContains1().also { instance = it }
                }
        }
        
        override fun getNewInstance(arg: List<*>, pathToItem: List<Any>, pathToSchemas: PathToSchemasMap): ItemsContainsList {
            val items: MutableList<Any?> = ArrayList()
            var i: Int = 0
            for (item: Any? in arg) {
            ...
            val newInstanceItems: FrozenList<Any?> = FrozenList(items)
            return ItemsContainsList(newInstanceItems)
        }
        
        @Throws(ValidationException::class)
        override fun validate(arg: List<*>, configuration: SchemaConfiguration?): ItemsContainsList {
            val pathSet: MutableSet<List<Any>> = HashSet()
            val pathToItem = listOf<Any>("args[0")
            val castArg = castToAllowedTypes(arg, pathToItem, pathSet)
            val usedConfiguration = configuration ?: SchemaConfiguration(JsonSchemaKeywordFlags.Builder().build())
        ...
        @Throws(ValidationException::class)
        override fun validateAndBox(arg: List<*>, configuration: SchemaConfiguration?): ItemsContains1BoxedList {
            return ItemsContains1BoxedList(validate(arg, configuration))
        }
        @Throws(ValidationException::class)
        override fun validateAndBox(arg: Any?, configuration: SchemaConfiguration?): ItemsContains1Boxed {
            if (arg is List<*>) {
                return validateAndBox(arg, configuration)
            }
            throw ValidationException("Invalid input type="+getClass(arg)+". It can't be validated by this schema")
