...
class TestFakeApi(ApiTestMixin):
    ...
    def test_array_model(self):
        ...
        with patch.object(RESTClientObject, "request") as mock_request:
            ...
            mock_request.return_value = self.response(self.json_bytes(json_data))
    ...
    def test_composed_one_of_different_types(self):
        ...
        # serialization + deserialization works
        number = composed_one_of_different_types.ComposedOneOfDifferentTypes.validate(
            10.0
        )
        ...
        none_instance = (
            composed_one_of_different_types.ComposedOneOfDifferentTypes.validate(None)
        )
        date_instance = (
            composed_one_of_different_types.ComposedOneOfDifferentTypes.validate(
                "1970-01-01"
            )
        )
    ...
    def test_body_with_query_params(self):
        ...
        with patch.object(RESTClientObject, "request") as mock_request:
            ...
            api_response = self.api.body_with_query_params(
                body=body, query_params={"query": "hi there"}
            )
            self.assert_request_called_with(
                mock_request,
    ...
    def test_upload_download_file_tx_bytes_and_file(self):
        ...
        with patch.object(RESTClientObject, "request") as mock_request:
            mock_request.return_value = mock_response
            body_info = request_body.RequestBodyInfoForApplicationOctetStream(
                file_bytes
            )
            api_response = self.api.upload_download_file(body_info=body_info)
            self.assert_request_called_with(
    ...
    @staticmethod
    def __get_streamable_body(
        file: typing.Union[io.BufferedReader, typing.BinaryIO]
    ) -> client.HTTPResponse:
        class FileMaker:
            @staticmethod
    ...
    def test_upload_download_file_rx_file(self):
        ...
        with patch.object(RESTClientObject, "request") as mock_request:
            ...
            api_response = self.api.upload_download_file(
                body_info=body_info, stream=True
            )
            self.assert_request_called_with(
                mock_request,
                "http://petstore.swagger.io:80/v2/fake/uploadDownloadFile",
        ...
        mock_response = self.response(
            ...
            content_type="application/octet-stream",
            headers={
                "content-disposition": f'attachment; filename="{saved_file_name}"'
            },
            preload_content=False,
        )
        with patch.object(RESTClientObject, "request") as mock_request:
        ...
        with patch.object(RESTClientObject, "request") as mock_request:
            no_filename_mock_response.geturl = (
                lambda: f"http://foo.bar/{expected_filename}"
            )
    ...
    def test_upload_file(self):
        ...
        try:
            with patch.object(RESTClientObject, "request") as mock_request:
                ...
                body_info = request_body.RequestBodyInfoForMultipartFormData(
                    {"file": file1}
                )
        ...
        with patch.object(RESTClientObject, "request") as mock_request:
            mock_request.return_value = self.response(self.json_bytes(response_json))
            body_info = request_body.RequestBodyInfoForMultipartFormData(
                {"file": file_bytes}
            )
            api_response = self.api.upload_file(body_info=body_info)
    ...
    def test_inline_composition(self, mock_request):
        ...
        mock_request.return_value = self.response(json_bytes)
        ...
        multipart_response = self.__encode_multipart_formdata(
            fields={"someProp": single_char_str}
        )
        mock_request.return_value = self.response(
        ...
        from petstore_api.paths.fake_inline_composition.post.responses.response_200.content.multipart_form_data import (
            schema,
        )
    ...
    def test_response_without_schema(self):
        ...
        with patch.object(RESTClientObject, "request") as mock_request:
            mock_request.return_value = self.response("blah", content_type="text/plain")
    ...
    def test_json_patch(self):
        with patch.object(urllib3.PoolManager, "request") as mock_request:
            ...
            from petstore_api.components.schema import (
